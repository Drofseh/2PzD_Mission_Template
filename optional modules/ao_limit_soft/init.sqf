["Soft AO Limit", "Allows the mission maker to set AO limits to specific sides.", "Olsen &amp; Starfox64"] call Olsen_FW_FNC_RegisterModule;
    ...
    if ((count _markers) > 0) then {
        ...
        [_markers] spawn {
            ...
            while {true} do {
                ...
                _allowedOutside = (!_air && {FW_AOTimer < 0}) || {_air && {FW_AOTimerAir < 0}};
                _outSide = true;
                ...
                if (_outSide) then {
                    if (!(_allowedOutside) && !_displayed && (_vehicle call Olsen_FW_FNC_Alive)) then {
