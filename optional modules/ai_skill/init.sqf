["AI skill", "Allows the mission maker to change the subskills of ai", "Sacher"] call Olsen_FW_FNC_RegisterModule;

Olsen_FW_FNC_setAISkill =
{
        ...
        if (_isstringcorrect && _value > 0.2 && _value <= 1) then {
            {
                ...
                _unit = _x;
                {
                    switch(_x select 0) do {
                        ...
                        case "Vehicle" : {
                            if (count _x == 1) then {
                                if (_unit call Olsen_FW_FNC_InVehicle) then {
                                    _condition = true;
                                    _conditionCheck set [count _conditionCheck,true];
                                } else {
                                    _conditionCheck set [count _conditionCheck,false];
