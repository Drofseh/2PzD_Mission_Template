["Cover Map Multiple AOs", "Covers map except specified area, allows switching between multiple AOs", "PIZZADOX"] call Olsen_FW_FNC_RegisterModule;
...
if (count _AOMarkers > 1) then {
    [{!isNull player}, {
        ...
        {
            call compile format ["
                private _MapChangeAction%1 = ['switch_MapAO', 'Switch Map to %2', '', {%4 call Olsen_FW_FNC_AOCoverAndCenterMap;}, {(visibleMap) && (FW_map_currentAO != %3) && (player getvariable ['HasAltMaps',false])}] call ace_interact_menu_fnc_createAction;
...
(_AOMarkers select 0) call Olsen_FW_FNC_AOCoverAndCenterMap;
