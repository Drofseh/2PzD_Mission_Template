["AI Attack", "Allows for continious ai attack", "Sacher"] call Olsen_FW_FNC_RegisterModule;

Olsen_FW_FNC_AtkRegisterPath = {
    ...
    {
        _x call Olsen_FW_FNC_NotTrackUnit;
        deleteVehicle _x;
    } forEach _units;
...
//example unit call Olsen_FW_FNC_AtkRegisterPath;

Olsen_FW_FNC_AtkSpawnCustomUnit = {
    private ["_newGroup","_unitToSpawn","_pos"];
    _newGroup = _this select 0;
    _pos = _this select 1;
    _unitToSpawn = _this select 2;
    _new = [_newGroup,_unitToSpawn select 0,([[[_pos,5]],[]] call BIS_fnc_randomPos)] call Olsen_FW_FNC_spawnAI;

    if (count _unitToSpawn >= 2) then {
        if (_unitToSpawn select 1 != "") then {[_new, _unitToSpawn select 1] call Olsen_FW_FNC_GearScript;};
    };
...
Olsen_FW_FNC_AtkRegisterUnit = {
    ATKcachedUnits pushBack (_this select 0);
    _this deleteAt 0;
    ATKcachedUnits pushBack _this;
};

Olsen_FW_FNC_AtkRegisterVehicle = {
    ATKcachedVehicles pushBack (_this select 0);
    _this deleteAt 0;
    ATKcachedVehicles pushBack _this;
};
//example ["Ins_Rif","O_Soldier_F","Ins_Rif"] call Olsen_FW_FNC_AtkRegisterUnit; [name,soldierclass,gearscript];

Olsen_FW_FNC_AtkRandomStart = {
        ...
        while {true} do {
            ...
            while {count _unitsSpawned < _maxAmmount} do {
                ...
                for "_i" from 0 to _ammountToSpawn do {
                        [_newGroup,_grpPos,_units call BIS_fnc_selectRandom] call Olsen_FW_FNC_AtkSpawnCustomUnit;
                        sleep 0.25;
                };
    ...
    _handle = _this spawn {
        ...
        while {true} do {
            ...
            for "_i" from 0 to _ammountToSpawn do {
                ...
                _veh = [(_unitToSpawn select 0), _grpPos,_side] call Olsen_FW_FNC_spawnVehicle;
                _crew = _unitToSpawn select 1;

                if (count _crew >= 1) then {
                       _new = [_newGroup,_grpPos, _crew select 0] call Olsen_FW_FNC_AtkSpawnCustomUnit;
                       _new moveInDriver _veh;
                       _new assignAsDriver _veh;
                };

                if (count _crew >= 2) then {
                       _new = [_newGroup,_grpPos, _crew select 1] call Olsen_FW_FNC_AtkSpawnCustomUnit;
                       _new moveInGunner _veh;
                       _new assignAsGunner _veh;
                };
                if (count _crew >= 3) then {
                   _new =  [_newGroup,_grpPos, _crew select 2] call Olsen_FW_FNC_AtkSpawnCustomUnit;
