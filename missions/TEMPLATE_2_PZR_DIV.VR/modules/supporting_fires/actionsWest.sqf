...
#define def_setFireMission(BOOL) \
    fireMissionAvailableWEST = BOOL; \
    publicVariable "fireMissionAvailableWEST";

#define def_removeShellsHE(NUMBER) \
    shellsHE_AmmoCountWEST = shellsHE_AmmoCountWEST - NUMBER; \
    publicVariable "shellsHE_AmmoCountWEST";

#define def_removeShellsSmoke(NUMBER) \
    shellsSmoke_AmmoCountWEST = shellsSmoke_AmmoCountWEST - NUMBER; \
    publicVariable "shellsSmoke_AmmoCountWEST";

#define def_removeShellsFlare(NUMBER) \
    shellsFlare_AmmoCountWEST = shellsFlare_AmmoCountWEST - NUMBER; \
    publicVariable "shellsFlare_AmmoCountWEST";

#define def_TargetVisual \
    _target = screenToWorld [0.5,0.5]; \
    previousTarget = _target; \
    shellDispersionWEST = originalShellDispersionWEST; \
    shellAccuracyWEST = originalShellAccuracyWEST; \
    repeatFire = 1; \
    _targetX = (_target select 0) + (random [-shellAccuracyWEST, 0, shellAccuracyWEST]); \
    _targetY = (_target select 1) + (random [-shellAccuracyWEST, 0, shellAccuracyWEST]);

#define def_Target01 \
    _target = getMarkerPos target01WEST; \
    previousTarget = _target; \
    shellDispersionWEST = originalShellDispersionWEST; \
    shellAccuracyWEST = originalShellAccuracyWEST; \
    repeatFire = 1; \
...
#define def_Target02 \
    _target = getMarkerPos target02WEST; \
    previousTarget = _target; \
    shellDispersionWEST = originalShellDispersionWEST; \
    shellAccuracyWEST = originalShellAccuracyWEST; \
    repeatFire = 1; \
...
#define def_Target03 \
    _target = getMarkerPos target03WEST; \
    previousTarget = _target; \
    shellDispersionWEST = originalShellDispersionWEST; \
    shellAccuracyWEST = originalShellAccuracyWEST; \
    repeatFire = 1; \
...
#define def_Target04 \
    _target = getMarkerPos target04WEST; \
    previousTarget = _target; \
    shellDispersionWEST = originalShellDispersionWEST; \
    shellAccuracyWEST = originalShellAccuracyWEST; \
    repeatFire = 1; \
...
#define def_Target05 \
    _target = getMarkerPos target05WEST; \
    previousTarget = _target; \
    shellDispersionWEST = originalShellDispersionWEST; \
    shellAccuracyWEST = originalShellAccuracyWEST; \
    repeatFire = 1; \
...
#define def_Target06 \
    _target = getMarkerPos target06WEST; \
    previousTarget = _target; \
    shellDispersionWEST = originalShellDispersionWEST; \
    shellAccuracyWEST = originalShellAccuracyWEST; \
    repeatFire = 1; \
...
#define def_TargetRepeat \
    ...
    repeatShellAccuracyWEST = shellAccuracyWEST / repeatFire; \
    shellDispersionWEST = shellDispersionWEST / repeatFire; \
    _targetX = (previousTarget select 0) + (random [-repeatShellAccuracyWEST, 0, repeatShellAccuracyWEST]); \
    _targetY = (previousTarget select 1) + (random [-repeatShellAccuracyWEST, 0, repeatShellAccuracyWEST]);

#define def_barrage5RoundsHE_WEST \
    _bomb = shellsHE_TypeWEST createVehicle [_targetX,_targetY,300]; \
    _bombPosition = _bomb getPos [random shellDispersionWEST, random 360]; \
...
#define def_barrage5RoundsSmoke_WEST \
    _bomb = shellsSmoke_TypeWEST createVehicle [_targetX,_targetY,300]; \
    _bombPosition = _bomb getPos [random shellDispersionWEST, random 360]; \
    _bombPositionX = (_bombPosition select 0); \
    _bombPositionY = (_bombPosition select 1); \
    _bomb setPos [_bombPositionX,_bombPositionY,300]; \
...
#define def_barrage1RoundFlare_WEST \
    _bomb = shellsFlare_TypeWEST createVehicle [_targetX,_targetY,300]; \
    _bombPosition = _bomb getPos [random shellDispersionWEST, random 360]; \
    _bombPositionX = (_bombPosition select 0); \
    _bombPositionY = (_bombPosition select 1); \
    _bomb setPos [_bombPositionX,_bombPositionY,300]; \
...
#define def_barrage5RoundsFlare_WEST \
    _bomb = shellsFlare_TypeWEST createVehicle [_targetX,_targetY,300]; \
    _bombPosition = _bomb getPos [random shellDispersionWEST, random 360]; \
    _bombPositionX = (_bombPosition select 0); \
    _bombPositionY = (_bombPosition select 1); \
...
_supportingFireActions = call {
    ...
    _conditionTargets      = {([] call acre_api_fnc_getCurrentRadio != "") && (fireMissionAvailableWEST) && (shellsHE_AmmoCountWEST >= 5 || shellsSmoke_AmmoCountWEST >= 5 || shellsFlare_AmmoCountWEST >= 5)};
    _conditionRepeat       = {([] call acre_api_fnc_getCurrentRadio != "") && (fireMissionAvailableWEST) && (!(previousTarget isEqualTo [])) && (shellsHE_AmmoCountWEST >= 5 || shellsSmoke_AmmoCountWEST >= 5 || shellsFlare_AmmoCountWEST >= 1)};
    _conditionFiresHE5     = {([] call acre_api_fnc_getCurrentRadio != "") && (fireMissionAvailableWEST) && (shellsHE_AmmoCountWEST >= 5)};
    _conditionFiresHE10    = {([] call acre_api_fnc_getCurrentRadio != "") && (fireMissionAvailableWEST) && (shellsHE_AmmoCountWEST >= 10)};
    _conditionFiresHE15    = {([] call acre_api_fnc_getCurrentRadio != "") && (fireMissionAvailableWEST) && (shellsHE_AmmoCountWEST >= 15)};
    _conditionFiresHE20    = {([] call acre_api_fnc_getCurrentRadio != "") && (fireMissionAvailableWEST) && (shellsHE_AmmoCountWEST >= 20)};
    _conditionFiresHE25    = {([] call acre_api_fnc_getCurrentRadio != "") && (fireMissionAvailableWEST) && (shellsHE_AmmoCountWEST >= 25)};
    _conditionFiresSmoke5  = {([] call acre_api_fnc_getCurrentRadio != "") && (fireMissionAvailableWEST) && (shellsSmoke_AmmoCountWEST >= 5)};
    _conditionFiresSmoke10 = {([] call acre_api_fnc_getCurrentRadio != "") && (fireMissionAvailableWEST) && (shellsSmoke_AmmoCountWEST >= 10)};
    _conditionFiresFlare1  = {([] call acre_api_fnc_getCurrentRadio != "") && (fireMissionAvailableWEST) && (shellsFlare_AmmoCountWEST >= 1)};
    _conditionFiresFlare5  = {([] call acre_api_fnc_getCurrentRadio != "") && (fireMissionAvailableWEST) && (shellsFlare_AmmoCountWEST >= 5)};
    _conditionCheck        = {([] call acre_api_fnc_getCurrentRadio != "") && (fireMissionAvailableWEST)};
    _conditionFiring       = {([] call acre_api_fnc_getCurrentRadio != "") && (!fireMissionAvailableWEST)};
    ...
    _statementRounds = {
        [] spawn {
            ...
            sleep 5;
            hint format ["%1 Rounds, HE,\n%2 rounds Smoke,\n%3 rounds Flare,\nremaining.", shellsHE_AmmoCountWEST, shellsSmoke_AmmoCountWEST, shellsFlare_AmmoCountWEST];
        };
    };
    ...
    _statementVisualLoc_5 = {
        ...
        [] spawn {
            ...
            */
            ...
            sleep 30;
            hint format ["Rounds complete.\n30 seconds to impact.\n%1 rounds remaining.", shellsHE_AmmoCountWEST];
            def_setFireMission(True);
            sleep 30;
            def_barrage5RoundsHE_WEST;
        };
    };
    ...
    _statementVisualLocS_5 = {
        ...
        [] spawn {
            ...
            hint "Roger, Fire Mission,\n5 Rounds, Smoke,\nOn your Mark.";
            sleep 30;
            hint format ["Rounds complete.\n30 seconds to impact.\n%1 rounds remaining.", shellsSmoke_AmmoCountWEST];
            def_setFireMission(True);
            sleep 30;
            def_barrage5RoundsSmoke_WEST;
        };
    };
    ...
    _statementVisualLocF_1 = {
        ...
        [] spawn {
            ...
            hint "Roger, Fire Mission,\n1 Round, Flare,\nOn your Mark.";
            sleep 30;
            hint format ["Rounds complete.\n30 seconds to impact.\n%1 rounds remaining.", shellsFlare_AmmoCountWEST];
            def_setFireMission(True);
            sleep 30;
            def_barrage1RoundFlare_WEST;
        };
    };
    ...
    if (target01WEST != "") then {
        _actionTarget01 = [target01WEST_Name,target01WEST_Name,"",_statementTargets,_conditionTargets] call ace_interact_menu_fnc_createAction;
    ...
    if (target02WEST != "") then {
        _actionTarget02 = [target02WEST_Name,target02WEST_Name,"",_statementTargets,_conditionTargets] call ace_interact_menu_fnc_createAction;
    ...
    if (target03WEST != "") then {
        _actionTarget03 = [target03WEST_Name,target03WEST_Name,"",_statementTargets,_conditionTargets] call ace_interact_menu_fnc_createAction;
    ...
    if (target04WEST != "") then {
        _actionTarget04 = [target04WEST_Name,target04WEST_Name,"",_statementTargets,_conditionTargets] call ace_interact_menu_fnc_createAction;
    ...
    if (target05WEST != "") then {
        _actionTarget05 = [target05WEST_Name,target05WEST_Name,"",_statementTargets,_conditionTargets] call ace_interact_menu_fnc_createAction;
    ...
    if (target06WEST != "") then {
        _actionTarget06 = [target06WEST_Name,target06WEST_Name,"",_statementTargets,_conditionTargets] call ace_interact_menu_fnc_createAction;
    ...
    _statementRepeat_5 = {
        ...
        [] spawn {
            ...
            sleep 30;
            def_barrage5RoundsHE_WEST;
            shellAccuracyWEST = repeatShellAccuracyWEST;
        };
    };
